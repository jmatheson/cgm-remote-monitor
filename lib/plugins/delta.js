...
function init() {
  ...
  delta.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (prop && prop.interpolated) {
      ...
      info.push({label: 'Absolute Delta', value: sbx.roundBGToDisplayFormat(sbx.scaleMgdl(prop.absMgdl)));
      info.push({label: 'Interpolated', value: sbx.roundBGToDisplayFormat(sbx.scaleMgdl(prop.mgdl5MinsAgo))});
    }
