...
function init() {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    _.forIn(prop.seenDevices, function seenDevice (device) {
      var deviceInfo = [device.name, device.status.symbol, device.status.label];
      if (device.mmtune) {
        var best = _.max(device.mmtune.scanDetails, function (d) {
          return d[2];
        });
...
function formatAgo (m, nowMills) {
  ...
  return (ago.value ? ago.value : '') + ago.shortLabel + (ago.shortLabel.length === 1 ? '' : '');
}
